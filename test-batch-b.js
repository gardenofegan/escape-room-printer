...
async function testBatchB() {
    console.log("Testing Batch B Puzzles...");
    const outDir = path.join(__dirname, 'test_output_b');
    if (!fs.existsSync(outDir)) fs.mkdirSync(outDir);

    const puzzles = [
        { type: 'NONOGRAM', generator: puzzleFactory.generateNonogramPuzzle.bind(puzzleFactory) },
        { type: 'CLOCK', generator: puzzleFactory.generateClockPuzzle.bind(puzzleFactory) },
        { type: 'DROP_QUOTE', generator: puzzleFactory.generateDropQuotePuzzle.bind(puzzleFactory) }
    ];
